...
- 商品详情采用纯文本说明与独立有序切片图集，复用统一资产；Admin 支持多选上传、保序重试、排序、批量移除和手机宽度预览，沿用 React 19、Ant Design 6 与 ProComponents，不新增依赖。后端补齐可信宽高与帧数、详情预算、引用删除保护及公开详情投影，OpenAPI 与 API Client 同步；本机开发库 `pinjie_mall_dev` 已在验证可读的迁移前备份后升级至 `20260926_01`，2 条历史商品图片元数据已回填，复核无待处理项。pytest、浏览器和小程序动态验证未执行。

- 增加零售电商公共规格属性库：清洗 `电商通用规格属性值库.xlsx` 的 1,036 条来源记录，排除 73 条本地生活资料，交付 104 个公共属性、801 个标准候选值及完整来源映射；开发库通过受控备份、清空重建和独立提交后核验完成导入。
- 补齐商品运营的公共属性筛选与批量启停、分类属性模板、描述属性集合维护、销售候选追加、SKU 组合维护和规格转换入口；OpenAPI 与生成 API Client 同步更新。
...
- 完成 2026-09-23 后端最新修复迁移与动态验收：本机开发库和隔离 PostgreSQL 测试库均升级至 `20260923_02`，通过空库、重复升级、备份恢复和 Alembic 模型漂移检查。修正财务审计迁移在新增 `target_revision` 前错误查询该列的问题，补齐 ORM 注释与外键排序元数据。完整 Backend pytest 441 项通过，行与分支综合覆盖率为 90.06%；新增真实 PostgreSQL 回归覆盖退款资金确认后的补偿回滚与重试幂等、重复或迟到收款异常退款、未决支付阻断资源释放。测试完成后测试库已恢复至测试前基线；真实渠道、常驻 Worker 部署、小程序和前端重型验收仍未执行。
...
- 修复 Admin 新建商品保存时因 SKU 规格映射未注册为表单字段而报空值转换异常：字段校验后读取完整表单数据，保留规格映射及规格转换元数据，并明确拒绝缺失或失效的组合。Admin typecheck、lint 通过；浏览器已复核服装分类两个颜色 SKU 的保存请求，出站前拦截并确认映射完整，未写入商品。Vitest、production build 与完整跨栈 E2E 未执行。
