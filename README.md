...
`pinjie-mall` 是独立维护、独立发布的商城仓库。用户端形态确定为微信小程序；Backend 的 M1 至 M4 商城领域已完成本地实现，Admin 运营页面和小程序工程仍由后续专项计划实施。

## 当前边界

- `apps/web` 是冻结保留的历史骨架，禁止开发、启动、构建、测试、发布和部署。
- 本地服务只允许 Backend 使用 8000 端口、Admin 使用 3001 端口。禁止监听或暴露 3000 及其他 Web 端口。
- Backend 已完成商品、库存、购物车、订单、支付售后和会员分销的 M1 至 M4 本地实现；真实数据库迁移、权限同步、渠道支付、退款、提现和重型测试仍需独立验证。
- Admin 当前已交付基础认证、用户、管理员、角色权限、安全日志和系统状态能力，商城运营页面仍需后续专项实施。
