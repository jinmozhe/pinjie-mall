...
class RefundRequest(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("amount > 0", name="ck_refund_request_amount"),
...
class RefundEvent(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("refund_request_id", "revision", name="uq_refund_event_revision"),
        CheckConstraint(
            "to_status IN ('requested', 'approved', 'rejected', 'processing', 'succeeded', 'unknown')",
            name="ck_refund_event_status",
        ),
        CheckConstraint("actor_type IN ('admin', 'user', 'payment')", name="ck_refund_event_actor"),
        {"comment": "退款申请状态事件"},
    )
...
class ReconciliationRecord(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("channel", "channel_transaction_id", name="uq_reconciliation_channel_transaction"),
