...
公开契约的破坏性变化必须先完成关联计划的消费者评估和迁移。确需精确例外时，在 `.oasdiff-exceptions.json` 按脚本要求登记负责人、批准日期、到期日、原因、迁移、上线、回滚与清理边界；计划中保留唯一 `oasdiff-exception` 标记及授权与迁移证据。

每条例外只允许匹配一个 HTTP 方法、完整路径和 oasdiff 原始诊断消息。日期按 UTC 校验，窗口最长三十天，截止日期当天即失效。通过 `pnpm check:openapi-exceptions` 校验后，CI 生成原生忽略文件并继续以 `fail-on: ERR` 拒绝所有未登记破坏性变化。禁止扩大忽略范围或修改门禁等级。

比较基线包含变更后立即清理对应登记，保留原计划；到期不得静默延期。该机制只豁免准确的静态诊断，不授权发布部署，也不替代消费者验证或数据库迁移。
