...
erDiagram
    ...
    MEMBER_LEVELS ||--o{ MEMBER_LEVEL_CONDITIONS : "后续条件"
    USERS ||--o{ MEMBERSHIP_QUALIFICATION_EVENTS : "后续资格事件"
    USERS ||--o{ MEMBER_LEVEL_EVENTS : "后续等级历史"
    USERS ||--o| POINTS_ACCOUNTS : "后续积分账户"
    POINTS_ACCOUNTS ||--o{ POINTS_LEDGERS : "后续积分流水"
```
...
67 张表按以下 15 个文档分组登记。配置物理表只在 07 计数，09 复用其配置行；15 中包含 1 张存量运费表和 5 张后续规划表。

| 模块序号 | 业务模块 | 表数量 | 包含数据表 | 状态性质 |
| --- | --- | --- | --- | --- |
| 01 | 用户身份与前台认证 | 4 | `users`, `user_sessions`, `user_refresh_tokens`, `user_external_identities` | 现有 3 表，小程序映射目标新增 |
...
| 15 | 限购控制、异步任务、评价对账与后续扩展 | 11 | `product_purchase_limits`, `product_purchase_records`, `durable_tasks`, `product_reviews`, `reconciliation_records`, `shipping_templates`（存量）, `member_level_conditions`, `membership_qualification_events`, `member_level_events`, `points_accounts`, `points_ledgers` | 现有、目标新增与规划分层 |
...
| `completed_at` | TIMESTAMPTZ | 是 | NULL | 渠道退款资金真正到账或内部零元售后完成的时间 |
| `created_at` | TIMESTAMPTZ | 否 | 应用当前 UTC 时间 | 创建时间 |
| `updated_at` | TIMESTAMPTZ | 否 | 系统写入 | 最近更新时间 |
...
用途：当前代码仍使用的商品运费模板。目标采用平台统一运费；完成迁移前不能宣布本表已退役。切换后仅用于有依据的存量解释，不参与目标新订单报价；退役步骤和旧字段见第 13 章。
...
miniapp_registration 的读取与首次用户/外部身份创建同事务取得共享配置锁；关闭操作与建号具有确定先后顺序。已有账户绑定要求可信原账户验证，不借注册开关自动合并。新分组均经显式迁移初始化、固定权限和固定接口维护，禁止万能 JSON 编辑接口。
...
申请状态为 requested → approved → completed，或 requested → rejected。自动审批仍保存申请和批准两个版本事件。approved 只代表审核通过；正额创建 after_sale 执行单及任务；零额直接记录内部售后完成，零个 refund_attempts，仍有后续追佣、库存、限购动作。
...
退款成功事务写执行 succeeded、资金事件、申请 completed、completed_at 和 refund_followup。后续独立幂等步骤包括：
...
| 退款资金确认 | 执行成功/事件、申请完成、跟进任务 | 退款网络调用 |
...
以下用例是实现阶段必须验证的条件，本次未执行应用测试、数据库测试、迁移、浏览器或真实渠道联调。Markdown、静态盘点和治理检查通过不等于这些用例通过。
...
| DB-24 | 退款补偿中断 | 资金成功后进程退出，后续任务只补未完成步骤；异常多收款退款不撤正常订单权益 |
...
| DB-37 | 迁移与存量 | 44 表现行数据可解释；旧两级整单佣金、部分退款、模板快照不伪造成新行级事实 |
| DB-38 | 全项目验收 | Backend/契约/Admin/小程序、真实 PostgreSQL 并发、渠道、调度告警及恢复全部有适用证据 |

“100%”只允许带分母和证据描述：本次表名登记覆盖当前仓库 44/44，原稿 66/66；本版 67/67 目标表有字典条目。字段级现行一致性、真实运行结构一致性、完整业务验收均不能由这些表名比例推导。
...
以下旧字段是现有实现事实，不属于目标新写入字段；必须在后续迁移计划记录负责人、期限、观测、删除测试及在途业务处理，不能作为无限期双轨。
...
1. 在同一后续全栈计划拆分基座和身份、商品/属性、统一价格/运费、订单/库存/限购、支付/售后、预算分佣/钱包、任务/对账、小程序与真实验收；关联原有 BASE 与 MP 编号。
