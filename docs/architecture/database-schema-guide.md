...
公共种子库：零售电商首期使用版本化种子 `apps/backend/scripts/data/spec_library.v1.json` 重建公共属性与标准候选值。导入只允许无商品采用历史的本机开发库，固定清空本模块七张表并先备份；分类、商品、SKU、库存不由种子预造。执行与恢复边界见[电商规格属性库初始化手册](../operations/spec-library-initialization.md)。
