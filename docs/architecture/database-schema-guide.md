...
> 文档状态：长期目标设计基线；本机开发库与隔离测试库已升级至 `20260923_02`，最新修复的 Backend 动态验证记录见第 13.1 节；真实渠道、小程序和完整运营闭环仍有未完成项；更新日期：2026-09-23。
>
...
| 商品到确定报价 | 分类/品牌/属性 → SKU → 会员/批发 → 统一运费 | 后端已有会员等级、统一价格规则、平台运费、结算预览与报价指纹；Admin 商品页仍显示迁移提示，新模型配置页面未全部适配 | 消费者适配及前后端业务验收；本机 Backend 证据见第 13.1 节 |
...
| 消费者和运营闭环 | Admin、Backend、生成契约、小程序 | Admin 已有运营查询、订单详情、接单、履约、提现及对账处置等入口；商品页仍在迁移，部分页面和状态筛选尚未完全适配新模型；小程序未建 | 剩余页面适配、两端业务验收、真实依赖与上线证据 |
