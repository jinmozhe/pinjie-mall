...
| 计划 | 状态 | 当前进度 |
| --- | --- | --- |
| [后端数据规划符合性分阶段修复计划](plans/2026-09-23_后端数据规划符合性分阶段修复计划.md) | 实施中 | 阶段 A 至 E 已完成本地实现；阶段 E 已补财务归属、提现全生命周期审计和版本化对账差异处置，后端、契约、Admin 与治理轻量门禁通过；实际迁移和动态验证未执行；阶段 F 待实施 |
